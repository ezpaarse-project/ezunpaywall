...
describe('Test POST 2 unpaywall resolver', () => {
  ...
  describe('POST: get unpaywall data with one DOI', () => {
    it(`should get unpaywall data - query ($dois: [ID!]!) {unpaywall(dois: $dois) { doi, is_oa } } - variables : { dois: [${doi1}] }`, async () => {
      const res = await chai.request(graphqlURL)
        .post('/graphql')
        .send({ query: 'query ($dois: [ID!]!) {unpaywall(dois: $dois) { doi, is_oa } }' })
        .send({
          variables: {
            dois: [doi1],
          },
    ...
    it('It should get empty tab - query ($dois: [ID!]!) {unpaywall(dois: $dois) { doi, is_oa } } - variable : { dois: [\'Coin coin\'] }', async () => {
      const res = await chai.request(graphqlURL)
        .post('/graphql')
        .send({ query: 'query ($dois: [ID!]!) {unpaywall(dois: $dois) { doi, is_oa } }' })
        .send({
  ...
  describe('POST: get unpaywall data with one DOI not normalized', () => {
    it(`should get unpaywall data - query ($dois: [ID!]!) {unpaywall(dois: $dois) { doi, is_oa } } - variables : { dois : ['${doi1.toUpperCase()}'] }`, async () => {
      const res = await chai.request(graphqlURL)
        .post('/graphql')
        .send({ query: 'query ($dois: [ID!]!) {unpaywall(dois: $dois) { doi, is_oa } }' })
        .send({
  ...
  describe('POST: get unpaywall data with two DOI', () => {
    it(`should get unpaywall data - query ($dois: [ID!]!) {unpaywall(dois: $dois) { doi, is_oa } } - variables : { dois : ${[doi1, doi2]} }`, async () => {
      const res = await chai.request(graphqlURL)
        .post('/graphql')
        .send({ query: 'query ($dois: [ID!]!) {unpaywall(dois: $dois) { doi, is_oa } }' })
        .send({
    ...
    it(`should get unpaywall data - query ($dois: [ID!]!) {unpaywall(dois: $dois) { doi, is_oa } } - variables : { dois : ${[doi1, 'Coin coin']} }`, async () => {
      const res = await chai.request(graphqlURL)
        .post('/graphql')
        .send({ query: 'query ($dois: [ID!]!) {unpaywall(dois: $dois) { doi, is_oa } }' })
        .send({
    ...
    it(`It should get empty tab - query ($dois: [ID!]!) {unpaywall(dois: $dois) { doi, is_oa } }  - variables : { dois : ${['Coin coin', 'Coin coin2']} }`, async () => {
      const res = await chai.request(graphqlURL)
        .post('/graphql')
        .send({ query: 'query ($dois: [ID!]!) {unpaywall(dois: $dois) { doi, is_oa } }' })
        .send({
