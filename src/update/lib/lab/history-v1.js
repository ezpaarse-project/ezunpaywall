...
/**
  * Gets difference value between 2 objects
  *
  ...
  */
function objectValueDiff(v1, v2, prefix = '') {
  const diffs = Object.keys(v1 ?? {}).reduce(
    (previous, key) => {
      ...
      if (typeof v1[key] === 'object') {
        return [
          ...previous,
          ...objectValueDiff(v1[key], v2[key], `${prefixedKey}.`),
        ];
      }
      if (v1[key] !== v2[key]) {
        return [
...
/**
  * Gets difference keys between 2 objects
  *
  * @param v1 First object to compare
  * @param v2 Second object to compare
  * @param prefix Prefix of returned keys. Should be empty on the first iteration
  ...
  * @returns Keys that are in v1 but not in v2
  */
function objectKeyDiff(v1, v2, prefix = '') {
  const diffs = Object.keys(v1 ?? {}).reduce(
    (d, key) => {
      const prefixedKey = `${prefix}${key}`;
      if (
...
async function insertData(listOfDoi, newData, date) {
  ...
  if (historyData.length > 0) {
    ...
    newData.forEach((data) => {
      ...
      if (oldDataUnpaywall) {
        const diffs = objectValueDiff(data, oldDataUnpaywall);
        if (diffs.length > 0) {
 ...
 * @param {string} insertConfig.date - Date of file inserted.
