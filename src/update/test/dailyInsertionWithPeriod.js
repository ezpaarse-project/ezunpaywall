...
describe('Test: download and insert file from unpaywall between a period', () => {
  ...
  describe(`Do a download and insert between ${date2} and ${date1}`, () => {
    ...
    it('Should return the process start', async () => {
      ...
      expect(res).have.status(200);
      expect(res.body.message).be.equal(`Dowload and insert snapshot from unpaywall from ${date2} and ${date1}`);
    });
  ...
  describe(`Don't download and insert between ${date4} and ${date3} because there is no file between these dates in ezunpaywall`, () => {
    ...
    it('Should return the process start', async () => {
      ...
      expect(res).have.status(200);
      expect(res.body.message).be.equal(`Dowload and insert snapshot from unpaywall from ${date4} and ${date3}`);
    });
